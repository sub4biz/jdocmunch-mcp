...
def _all_tools() -> list[Tool]:
    ...
    return [
        Tool(
            name="index_local",
            description="Index a local folder containing documentation files (.md, .txt, .rst). Parses by heading hierarchy into sections for efficient retrieval. Embeddings auto-enable when a provider is configured (GOOGLE_API_KEY, OPENAI_API_KEY, explicit OpenAI-compatible config, or sentence-transformers).",
            inputSchema={
                "type": "object",
                "properties": {
                    "path": {
                    ...
                    "use_embeddings": {
                        "description": "Generate semantic embeddings for each section, enabling hybrid (BM25+semantic) search. true/false/\"auto\". \"auto\" (default) enables embeddings when an embedding provider is configured (GOOGLE_API_KEY, OPENAI_API_KEY, explicit OpenAI-compatible config, or sentence-transformers installed).",
                        "default": "auto"
                    },
        ...
        Tool(
            name="doc_index_repo",
            description="Index a GitHub repository's documentation. Fetches .md/.txt files, parses sections, and saves to local storage. Embeddings auto-enable when a provider is configured (GOOGLE_API_KEY, OPENAI_API_KEY, explicit OpenAI-compatible config, or sentence-transformers).",
            inputSchema={
                "type": "object",
                "properties": {
                    "url": {
                    ...
                    "use_embeddings": {
                        "description": "Generate semantic embeddings for each section. true/false/\"auto\". \"auto\" (default) enables embeddings when an embedding provider is configured, including explicit OpenAI-compatible config.",
                        "default": "auto"
                    },
                    "incremental": {
                        "type": "boolean",
